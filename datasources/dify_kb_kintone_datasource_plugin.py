...
def _resolve_page_parameters(page: Any) -> Mapping[str, Any]:
    ...
    where: module scope helper near datasource definition
    what: Normalises different SDK shapes so we always hand back a mapping.
    why: Older dify-plugin releases omitted ``datasource_parameters`` and broke direct access.
    """
...
class DifyKbKintoneDatasourcePluginDataSource(OnlineDocumentDatasource):
    ...
    def _get_content(
        ...
        page_parameters = _resolve_page_parameters(page)
        ...
        debug_enabled = self._is_debug_enabled(page_parameters)
        self._debug_log(
            debug_enabled,
